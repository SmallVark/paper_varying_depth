...
As the next step we choose a fiducial cosmology $\b \Phi$ and compute reference correlation functions $\xi_\pm^{ij}(\theta,\b\Phi)$ for each pair of redshift bins $i,j$. Using our semi-analytic model in Sect.~\ref{sec:xipm_semianalytic}, we derive the observed correlation function $\xi_\pm^{ij,\rm{obs}}(\theta,\b\Phi)$ from those. Then we run a Markov-Chain Monte Carlo simulation to check for the resulting bias in the inferred cosmological parameters, using the covariance-matrix computed in H18. As our main interest lies in the $\Omega_{\rm m}$ - $\sigma_8$ combination, we restrict our analysis to those two parameters. All other parameters are taken to be fixed as the best-fit values from H17. As can be seen in Fig.~\ref{fig:mcmc_kids}, the impact of varying depth is insignificant compared to the uncertainties. To get a rough estimate for the impact on future surveys, we divide our covariance-matrix by a factor of 30, to approximately account for the increased survey area of LSST and Euclid with respect to KV450. As can be seen in Fig.~\ref{fig:mcmc_euclid}, here the impact on both $\Omega_{\rm m}$ and $\sigma_8$ is significant at the level of approx.~$1\sigma$. Conveniently it seems that the parameter $S_8$ is considerably more robust against this effect.

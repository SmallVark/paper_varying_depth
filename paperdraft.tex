...
\end{subfigure}
\caption{Recovered cosmological parameters for a variable-depth (Observed) and uniform-depth (Reference) KiDS-$450$-like (left) and KiDS-$15\,000$-like survey (right). The left simulation was computed using the covariance matrix of H18. For the KiDS$15\,000$-like survey, we divided the one of H18 by 30. This approximately accounts for the increased survey area of next-generation experiments, but does not factor in the increased number density and better shape measurements, so that this figure constitutes a conservative estimate on this effect for Euclid and LSST. Both figures were computed using a fiducial cosmology of $\Omega_{\rm{m}}=0.25$ and $\sigma_8 = 0.85$.}
